...
namespace octet {
      class  _lsystem  : public resource {
	...
	public:
		...
		/*	printf("%i\n", space_needed(0, 0));
			...
			space = space_needed(0, 0);
			unsigned int prev_iteration_length = iteration[0].size();
			iteration[1].reserve(space);
			for (unsigned int i = 0; i < prev_iteration_length; ++i)

				if (iteration[0][i] = 'F') {
					int rule_length = strlen(rule[0]);
					for (int j=0; j < rule_length; j++) {
						iteration[1].push_back(rule[0][j]);
					}
				}
				else {
					iteration[1].push_back(iteration[0][i]);
				}
				//debug
				for (unsigned int i = 0; i < iteration[1].size(); ++i) {
					printf("%c", iteration[1][i]);
				}
				printf("\n"); */
